/**
 * One-shot DB setup over Neon's HTTP (serverless) driver — for environments
 * where the Postgres wire protocol (TCP :5432) is blocked but HTTPS works.
 * Runs migrations then seeds categories, proprietary tools, and listings.
 *
 *   DATABASE_URL=... npx tsx src/lib/db/setup-http.ts
 */
...
async function main() {
  ...
  console.log('→ Applying migrations (HTTP) ...')
  ...
  await db
    .insert(categories)
    .values(CATEGORIES.map((c) => ({ slug: c.slug, name: c.name, description: c.description, sortOrder: c.sortOrder })))
    .onConflictDoUpdate({
      target: categories.slug,
      set: { name: sql`excluded.name`, description: sql`excluded.description`, sortOrder: sql`excluded.sort_order` },
    })
  ...
  // Insert in batches (HTTP driver caps payload/statement size).
  console.log(`→ Upserting ${values.length} listings (batched) ...`)
  const BATCH = 25
  ...
  console.log(`\n✓ Setup complete. ${Number(count)} listings in database.`)
}
